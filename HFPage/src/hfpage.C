...
void HFPage::init(PageId pageNo)
{
    ...
    this->freeSpace=MAX_SPACE - DPFIXED+sizeof(this->slot[1]);
...
Status HFPage::insertRecord(char* recPtr, int recLen, RID& rid)
{
     ...
     //   cout<<"fill rercord data "<<this->data[first_Insert_ptr]<<"  recptr"<<recPtr<<endl;
        this->usedPtr=first_Insert_ptr;   //set usedPtr 
        if(this->slotCnt==1)
        {
            this->slot[1].length=recLen;
            this->slot[1].offset=first_Insert_ptr;
        }
        else
        {
...
Status HFPage::deleteRecord(const RID& rid)
{
      ...
      if(rid.slotNo==0) 
       {
            this->freeSpace=(this->freeSpace)+this->slot[1].length;
            if(this->slotCnt==(rid.slotNo+1))
              this->usedPtr=this->usedPtr+this->slot[1].length;  // modify first usedPtr when only one record exit 
              memset(&data[this->slot[1].offset],this->slot[1].length,0);    // recycle memory 
            this->slot[1].length=-1;
            return OK;
       ...
       else
      {
         ...
         this->freeSpace=(this->freeSpace)+rid_slot->length+Slot_size;
           if(this->slotCnt==(rid.slotNo+1))
              this->usedPtr=this->usedPtr+this->slot[1].length;   //modify last recrod for this usePtr
           rid_slot->length=-1;
...
Status HFPage::nextRecord (RID curRid, RID& nextRid)
{
    ...
    short Begin_slot_address,record_offset,i;
    char slot_char[Slot_size];
    for(i=curRid.slotNo;i<this->slotCnt;i++)
    {
          Begin_slot_address=i*Slot_size;      // next slot in the array
          memcpy(slot_char,&data[Begin_slot_address],Slot_size);   //get slot
          slot_t *rid_slot=(slot_t *)(slot_char);
...
Status HFPage::getRecord(RID rid, char* recPtr, int& recLen)
{
     ...
     cout<<"offset="<<rid_slot->offset<<" length "<<rid_slot->length<<endl;
      ...
      cout<<"offset 0="<<this->slot[1].offset<<" length 0 "<<this->slot[1].length<<endl;
#endif 
     if(rid.slotNo>=1)
     {
     ...
     {
        memcpy(recPtr,&(this->data[this->slot[1].offset]),rid_slot->length); 
        recLen=this->slot[1].length;
     }
  //    cout<<"record data "<<recPtr<<endl;
    // fill in the body
     return OK;
}
...
Status HFPage::returnRecord(RID rid, char*& recPtr, int& recLen)
{
     ...
     {
        recLen=this->slot[1].length;
        recPtr=&this->data[slot[1].offset];
     }
    // fill in the body
    return OK;
}
