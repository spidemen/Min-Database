...
void BTreeTest::test1()
{
    ...
    cout << "\nstart BTreeIndex insertion" << endl
         << endl;
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (i % divisor == 0)
        {
            kill[(i / divisor)].r.pageNo = rid.pageNo;
            kill[(i / divisor)].r.slotNo = rid.slotNo;
            kill[i / divisor].key = key;
        }
    ...
    cout << "\nstart BTreeIndex deletion" << endl
         << endl;
    int j = 0;
    for (i = 0; i < num; i++)
    {
        ...
        {
            j++;
            if (btf->Delete(&kill[i / divisor].key, kill[(i / divisor)].r) != OK)
            {
                cout << " Deleting record with key = " << kill[i / divisor].key << "  [pageNo,slotNo] = ";
                cout << "[" << kill[i / divisor].r.pageNo << "," << kill[i / divisor].r.slotNo << "] failed !!" << endl;
                minibase_errors.show_errors();
            }
        }
    }
    ...
    if (status == OK)
        cout << "\n BTreeIndex opened successfully." << endl
             << endl;
...
void BTreeTest::test3()
{
    ...
    keysamples.open("new_testcase", ios::in);
    if (!keysamples)
    {
        cout << "keys not found.\n";
    ...
    while (!keysamples.eof())
    {
        ...
        if (i == 100)
            strncpy(lokey, key, keysize);
        //if(i==100) strncpy(hikey,key,keysize);
        if (i == 200)
            strncpy(hikey, key, keysize);
        keysamples.getline(key, keysize, '\n');
    }
...
void BTreeTest::test4()
{
    ...
    cout << "\nBTreeIndex created successfully." << endl
         << endl;
	...
	values[i].sort_value1 = rand() % 1000000;
        ...
        values[i].key = i * 16 + (rand() % 16);
        //values[i].key = i;
        values[i].r.pageNo = i;
        values[i].r.slotNo = i + 1;
        values[i].sort_value1 = rand() % 200000;
        values[i].sort_value2 = rand() % 200000;
        /*
