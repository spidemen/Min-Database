...
char file_extra[]="extra_test";

AttrType 	attrType[] = { attrInteger, attrString };
AttrType 	attrType1[] = { attrString,attrInteger };
short		attrSize[] = { 4, 4 };
...
void createFiles()
{
	...
	RID		rid;
		...
		HeapFile*	f1 = new HeapFile(file_extra,s);
		if (s != OK)
			cout<<"this is ridiculous !"<<endl;
		for (int j=4; j<28; j=j+2)
		{
			rec1.key =j;
			s = f1->insertRecord((char*)&rec1,sizeof(rec),rid);
			if (s != OK)
				cout<<"this is ridiculous !"<<endl;
		}
		delete f1;
...
Status test(int t)
{
	...
	if(t>7) sprintf(outfile,"test_extra2");
	else
	sprintf(outfile,"test%d", t);

	// Perform sort-merge on R and S
 	if(t>7)
 		sortMerge	sm(files[0],NUM_COLS,attrType,attrSize,JOIN_COL,file_extra,NUM_COLS,attrType1,attrSize,1,outfile,SORTPGNUM,Ascending,s);
 	else
	...
	RID	rid;
		char	rec[sizeof(struct _rec)*2];
			char	rec1[sizeof(struct _rec1)*2];
	if(t<=7)
	{
	...
	else
	{
		...
		cout << endl;
		cout << "------------ Test " << t << " ---------------" << endl;
		for (s = scan->getNext(rid, rec1, len); s == OK; s = scan->getNext(rid, rec1, len)) 
		{
	  	cout << (*((struct _rec*)&rec1)).key << "\t" << (*((struct _rec1*)&rec1[8])).key << endl;
		}
		cout << "-------- Test " << t << " completed --------" << endl;
	}
	delete scan;
...
int SMJTester::test2()
{
	...
	RID		rid;
	char file_extra[]="extra_test";
		HeapFile*	f = new HeapFile(file_extra,s);
		if (s != OK)
			cout<<"this is ridiculous !"<<endl;
		for (int j=4; j<28; j=j+2)
		{
	...
	sprintf(outfile,"test_extra2");
	// Perform sort-merge on R and S
	sortMerge	sm(files[0],NUM_COLS,attrType,attrSize,JOIN_COL,file_extra,NUM_COLS,attrType,attrSize,1,outfile,SORTPGNUM,Ascending,s);
	if (s != OK)
	{
		cout << "Test extra "<< " -- sortMerge failed" << endl;
		return s;
	}
	...
	if (s != OK)
	{
		cout << "Test  exra"<< " -- result file not created " << endl;
		return s;
	}
     Scan*	scan = outf->openScan(s);
	assert(s == OK);
	int len;
	char	rec1[sizeof(struct _rec)*2];
	cout << endl;
	cout << "------------ Test  extra" << " ---------------" << endl;
	for (s = scan->getNext(rid, rec1, len); s == OK; s = scan->getNext(rid, rec1, len)) 
	{
	  cout << (*((struct _rec1*)&rec1)).key << "\t" << (*((struct _rec1*)&rec1[8])).key << endl;
	}
	cout << "-------- Test  exra" << " completed --------" << endl;
	delete scan;
	s=outf->deleteFile();
	if(s!=OK) MINIBASE_CHAIN_ERROR(JOINS,s);
	delete outf;
