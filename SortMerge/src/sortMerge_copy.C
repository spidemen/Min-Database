...
sortMerge::sortMerge(
...
#if 1
         Status hp;
         HeapFile * f_in1 = new HeapFile(filename1,hp);
        if(hp!=OK) cout<<"Fail: create heapfile "<<filename1<<endl;
         HeapFile * f_in2 = new HeapFile(filename2,hp);
        if(hp!=OK) cout<<"Fail: create heapfile "<<filename2<<endl;
         HeapFile  *f_out = new HeapFile(filename3,hp);
        if(hp!=OK) cout<<"Fail: create heapfile "<<filename3<<endl;
#endif 
         short length=0;
    ...
    /*
            ...
            {
              //  cout<<"map test"<<rec_arry2[j].key<<endl;
